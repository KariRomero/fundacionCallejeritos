...
const Carousel = () => {
    ...
    const [currentIndex, setCurrentIndex] = useState(0);
    const [cardsToShow, setCardsToShow] = useState(4); // Inicialmente mostrar 4 tarjetas
    ...
    const next = () => {
        setCurrentIndex((prevIndex) =>
            (prevIndex + 1) % adoptions.length
        );
    };

    const prev = () => {
        setCurrentIndex((prevIndex) =>
            (prevIndex - 1 + adoptions.length) % adoptions.length
        );
    };

    const displayedCards = adoptions.slice(currentIndex, currentIndex + cardsToShow);
